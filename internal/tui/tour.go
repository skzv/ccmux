...
func defaultTourSteps() []tourStep {
	return []tourStep{
		...
		},
		{
			...
			Body: []string{
				"The Dashboard is your at-a-glance home. It shows:",
			},
			Bullets: []string{
				"Every Claude session across every project, color-coded by state",
				"5-hour usage panel (token totals + prompts remaining on Pro/Max)",
				"Per-project breakdown of where your tokens went today",
				"Daemon + remote-host health on the right",
			},
			KeyHint: "Press 1 / F1 anywhere to come back here",
		},
		{
			Title: "Sessions screen (2)",
			Body: []string{
				"Sessions is the actionable view. From here you can:",
			},
			Bullets: []string{
				"Enter — attach (ccmux re-execs into tmux for you)",
				"n — new session for the current project",
				"x — kill the highlighted session (asks first)",
				"R — rename  ·  k — pin keep-awake  ·  s — snapshot",
			},
