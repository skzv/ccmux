...
Below the narrow breakpoint the Home screen SHALL render as a single full-width column, stacked top to bottom: hero → sessions → session summary → devices → usage. At or above the breakpoint it SHALL render two columns — the sessions list and detail on the left half, and the hero plus the three stat tiles (session summary, devices, usage) stacked on the right half.

#### Scenario: Single column below the breakpoint
- **WHEN** the Home screen renders at width < 120
- **THEN** the hero, sessions, session summary, devices, and usage render in one full-width column in that top-to-bottom order

#### Scenario: Two columns at or above the breakpoint
- **WHEN** the Home screen renders at width ≥ 120
- **THEN** the sessions list and detail occupy the left half
- **THEN** the hero and the three stat tiles occupy the right half
- **THEN** no rendered line exceeds the terminal width
