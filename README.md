...
One TUI for every Claude / Codex / Antigravity / Cursor session across every device on your tailnet. No tmux session names to memorize, no `claude --resume <hash>` to type, no SSH-then-attach gymnastics when you pick up on your phone. Your work follows you between devices — frictionless.
...
Pick per project which AI runs it — [Claude Code](https://claude.ai/code), [Codex](https://github.com/openai/codex), [Antigravity CLI](https://antigravity.google/download), or [Cursor](https://cursor.com/cli). The choice is sticky, stored at `<project>/.ccmux/agent`. The dashboard, daemon state-detection, and dispatch all follow per-project. Press `a` in the Projects tab to switch the selected project's agent (cycles claude → codex → antigravity → cursor).

Dashboard rows on non-default agents get a small `[codex]`, `[antigravity]`, or `[cursor]` tag so a single glance tells you what's running where.
...
- Dashboard rows on non-default agents get a small `[codex]`, `[antigravity]`, or `[cursor]` tag
