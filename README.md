...
- **Conversations list hides automation noise by default.** Headless / SDK Claude runs (`claude -p`, automation wrappers) are tagged with `entrypoint: "sdk-cli"` in the transcript and filtered out of the Conversations tab, the project menu, and `ccmux list-conversations` — so a user who wires Claude into scripts doesn't drown in one-shot rows. Press `H` in the Conversations screen to toggle them back on, pass `--include-headless` to the CLI, or set `[conversations] show_headless = true` in `~/.config/ccmux/config.toml` to opt back in permanently.
