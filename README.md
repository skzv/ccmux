...
- **Adopt an existing directory.** Press `A` in the Projects tab to register a directory under your projects root that has no `.git`, no `CLAUDE.md`, and no `.ccmux/` marker — handy for a worktree, an extracted tarball, or a scratch dir you want to start managing. `ccmux adopt <name>` does the same from the CLI.
